...
def cart_action(request, user_pk):
    if request.method == "POST":
        ...
        if action == 'delete':
            ...
            Cart.objects.filter(user_id=user_pk, product_id__in[selected_products]).delete()
        elif action == 'checkout':
            # Handle the checkout process for selected products
            # Redirect to the checkout page or process the payment
            return redirect(reverse('shop:checkout') + '?products=' + ','.join(selected_products))
